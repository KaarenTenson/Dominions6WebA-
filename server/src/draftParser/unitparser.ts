...
function createRandomMagicPaths(
...
export function parseUnitsCsv(filePath: string): Unit[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    const unit: Unit = {
      ...
      commander: toBoolean(row.commander),
      capitalOnly: toBoolean(row.capital_only),
